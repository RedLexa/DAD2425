...
public class DadkvsMainServiceImpl extends DadkvsMainServiceGrpc.DadkvsMainServiceImplBase {
	...
	if(server_state.i_am_leader && !server_state.request_list.containsKey(request.getReqid())){
		// comecar uma fase 1, olhando para o seu seq_number interno e verificando qual deve o proximo index a executar, propondo-o dessa forma

		//server_state.req_to_propose = last_requestid_received; TO-DO: arranjar maneira de saber sempre qual a transacao que o lider acha que devia ser executada
		...
		GenericResponseCollector<DadkvsPaxos.PhaseoneReply> phase_one_collector = new GenericResponseCollector<DadkvsPaxos.PhaseOneReply> (phase_one_responses, n_servers);
		for (int i = 0; i < n_servers; i++) {
				if(server_state.my_id == i){
					//this.server_state.phase_two_requests.put(request.getReqid(), phase_two_request.build());
					continue;
				}
				CollectorStreamObserver<DadkvsPaxos.PhaseOneReply> phase_one_observer = new CollectorStreamObserver<DadkvsPaxos.PhasOneReply>(phase_one_collector);
				async_stubs[i].phaseone(phase_one_request.build(), phase_one_observer);
			}
		int all_responses = n_servers - 1;
		phase_one_collector.waitForTarget(all_responses);
	...
	server_state.timestamp++;
	TransactionRecord txrecord = new TransactionRecord (key1, version1, key2, version2, writekey, writeval, server_state.timestamp);
